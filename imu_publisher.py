    ...
    def start_stream(self):
        ...
        def transform_pose_quat(position, orientation, from_frame, to_frame, stamp):
            if self.tf_buffer is None or to_frame is None or to_frame == "Select frame...":
                return position, orientation, True  # True = no error
            if not from_frame:
            ...
            try:
                ...
                if not self.tf_buffer.can_transform(to_frame, from_frame, stamp):
                    print(f"No transform from {from_frame} to {to_frame} at time {stamp}")
                    return position, orientation, False
